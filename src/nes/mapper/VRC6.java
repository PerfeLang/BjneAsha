...
public class VRC6 extends MapperAdapter {
    ...
    public void write(short sadr, byte sdat) {
        ...
        switch (adr & 0xF003) {
            ...
            case 0xB003:
                switch ((dat >> 2) & 3) {
                    case 0: // Horizontal mirroring
                        nes.getPpu().setMirroring(Ppu.VERTICAL);
                    break;
                    case 1: // Vertical mirroring
                        nes.getPpu().setMirroring(Ppu.HOLIZONTAL);
                    break;
            ...
            case 0xB001:
            case 0xB002:
                writeQueue.push(new WriteDat(nes.getCpu().getMasterClock(), sadr, sdat));
                while (writeQueue.size() > 1000) {
                    WriteDat wd = (WriteDat) writeQueue.pop();
    ...
    public void audio(SoundInfo info) {
        ...
        for (int i = 0; i < info.sample; i++) {
            ...
            while (!writeQueue.isEmpty() && cur >= ((WriteDat)writeQueue.peek()).clk) {
                WriteDat wd = (WriteDat) writeQueue.pop();
                sndWrite(wd.adr, wd.dat);
            }
