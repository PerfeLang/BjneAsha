...
public class Cpu {
    ...
    public void exec(int clk) {
        ...
        do {
            ...
            switch (opc & 0xff) {
                ...
                default:
                    System.out.println("undefined opcode: " + (opc & 0xff));
                break;
            }
        } while (rest > 0);
    }
    ...
    private final static int NMI = 0;
    private final static int IRQ = 1;
    private final static int RESET = 2;

    private void execIrq(int it) {
        if (logging)
            System.out.println((it == RESET ? "RESET" : it == NMI ? "NMI" : "IRQ") + " occured!");

        short vect = (short) ((it == RESET) ? 0xFFFC
                : (it == NMI) ? 0xFFFA : (it == IRQ) ? 0xFFFE
                        : 0xFFFE);
...
/*
    ...
    private void log() {
        ...
        System.out.println("%06d: %04d: %04X %-13s : A:%02X X:%02X Y:%02X S:%02X P:%c%c%c%c%c%c%c\n\", cnt++, rest, regPC & 0xffff, s, regA & 0xff, regX & 0xff, regY & 0xff, regS & 0xff, nFlag != 0 ? 'N' : 'n', vFlag != 0 ? 'V' : 'v', bFlag != 0 ? 'B' : 'b', dFlag != 0 ? 'D' : 'd', iFlag != 0 ? 'I' : 'i', zFlag != 0 ? 'Z' : 'z', cFlag != 0 ? 'C' : 'c'");
    }
